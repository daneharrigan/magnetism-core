...
def run_spec_file(file)
  system('clear')
  result = run(%Q(rspec -I spec/app_root/spec #{file}))
  growl result.split("\n").last rescue nil
  puts result
end
...
watch('spec/app_root/spec/spec_helper\.rb') { run_all_specs }
watch('spec/app_root/spec/.*/.*_spec\.rb') { |m| run_spec_file(m[0]) }
watch('spec/app_root/spec/.*/.*/.*_spec\.rb') { |m| run_spec_file(m[0]) }
